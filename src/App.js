    ...
    <>
      ...
      <Main>
        <Box>
          ...
          {error && <ErrorMessage message={error} query={query} />}
        </Box>
        <Box>
          {selectedId ? (
            <MoviesDetails
...
function ErrorMessage({ message, query }) {
  return (
    <p className="error">
      <span>{query.length === 0 ? "🔍" : "⚠️"}</span> {message}
    </p>
  );
}
...
function Movies({ movie, onSelectMovies }) {
  return (
...
function MoviesDetails({ selectedId, onCloseMovies, onAddWatched }) {
